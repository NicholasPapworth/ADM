...
def load_workbook(uploaded_file=None) -> pd.DataFrame:
    ...
    def compute_group_metrics(group: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ratio_columns:
            ...
            # Remove the '_ratio' suffix to derive base fertiliser name
            base_name = col[:-6] if col.endswith('_ratio') else col
            # z‑score: (value – mean) / std
            group[f"{base_name}_z"] = (series - mean) / std
            # percentile rank: fraction of values less than or equal to current
            group[f"{base_name}_percentile"] = series.rank(pct=True)
            # indexed affordability: first value normalised to 100
            base = series.iloc[0]
            group[f"{base_name}_index"] = (series / base) * 100 if pd.notnull(base) else np.nan
        # Basket metrics
